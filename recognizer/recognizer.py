...
def recognizer(details):
    ...
    for root, dirs, files in image_dir:
        for f in files:
            if f.endswith('jpg') or f.endswith('png'):
                ...
                img = face_recognition.load_img_file(path)
    ...
    width = int(cap.get(cv2.CAP_FRAME_PROP_WIDTH))
    height = int(cap.get(cv2.CAP_FRAME_PROP_HEIGHT))
    ...
    while True:
        ...
        if ret:
            ...
            video_face_encoding = face_recognition.face_encoding(frame,
                                        video_face_location,
                                        tolerance=0.6
                                    )
    ...
    return je_video_ma_malya_enu_naam
